...
const AiModelManagementPage = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Epoch training logs tracker console */}
      {logs.length > 0 && (
        <div className="rounded-2xl border border-slate-205 bg-slate-900 text-slate-100 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2 text-xs font-bold text-slate-400 tracking-wider">
            <span>Weights Compiler Training Console logs</span>
            <span className="text-[10px] bg-slate-850 px-2 py-0.5 rounded text-emerald-500 animate-pulse">Running compilation epoch</span>
          </div>
          <div className="font-mono text-[11px] leading-relaxed space-y-1 overflow-y-auto max-h-40">
            {logs.map((log, idx) => <p key={idx} className={log.includes('accuracy') || log.includes('ready') ? 'text-emerald-450' : 'text-slate-400'}>{`$ ${log}`}</p>)}
          </div>
        </div>
      )}
    </div>
